...
describe("performReview: review-cache memoization end to end", () => {
  ...
  /**
   * Tranche dispatch (2026-08-05 wave, cycle 3). The engine's own `--max-tokens-budget` was
   * MEASURED to gate individual LLM calls, not file dispatches (a 25k budget reviewed all five
   * corpus files for 273k–341k; a 1-token budget spent nothing yet counted every file as
   * reviewed), so the adapter now dispatches in tranches and checks REAL spend between them —
   * see `runEngineInTranches`'s own doc comment for the three rules these tests pin.
   *
   * Runs last-ish in this file for the same reason the gate block does: it adds one commit on top
   * of the shared fixture repo, which leaves every sha earlier tests captured untouched.
   */
  describe("performReview: tranche dispatch (v0.15.0)", () => {
    // concurrency 1 -> tranche size 2 (TRANCHE_CONCURRENCY_FACTOR), so four files make two
    // tranches without needing a large fixture.
    const TRANCHE_CONFIG: RuntimeConfig = { ...CONFIG, concurrency: 1 };
    // Its own throwaway repository, not the shared fixture. This block asserts on the NUMBER of
    // engine invocations, which is a function of how many reviewable paths the diff carries — and
    // the shared repo accumulates a commit per fixture block above, so borrowing it made the
    // tranche count depend on unrelated tests (measured: 9 reviewable paths instead of 4, five
    // tranches instead of two). The rename block one level up keeps its own repo for the same
    // reason.
    let trancheRepo = "";
    let trancheBase = "";
    let trancheHead = "";
    ...
    beforeAll(async () => {
      trancheRepo = await mkdtemp(join(tmpdir(), "kfq-tranche-"));
      git(["init", "-q", "-b", "main"], trancheRepo);
      await mkdir(join(trancheRepo, "src"), { recursive: true });
      await writeFile(join(trancheRepo, "src/keep.ts"), "export const keep = 1;\n");
      git(["add", "-A"], trancheRepo);
      git(["commit", "-q", "-m", "base", "--no-gpg-sign"], trancheRepo);
      trancheBase = git(["rev-parse", "HEAD"], trancheRepo).trim();

      for (const n of [1, 2, 3, 4]) {
        await writeFile(
          join(trancheRepo, `src/t${String(n)}.ts`),
          `export const t${String(n)} = 1;\n`,
        );
      }
      git(["add", "-A"], trancheRepo);
      git(["commit", "-q", "-m", "four reviewable files", "--no-gpg-sign"], trancheRepo);
      trancheHead = git(["rev-parse", "HEAD"], trancheRepo).trim();
    });

    afterAll(async () => {
      if (trancheRepo !== "") await rm(trancheRepo, { recursive: true, force: true });
    });

    function trancheRequest(cacheStore?: CacheStore): ReviewRequest {
      const client = new GitHubClient("https://api.example.test", "unused");
      vi.spyOn(client, "getPullRequest").mockResolvedValue({
        headSha: commitSha(trancheHead),
        draft: false,
        baseRef: "dev",
        headRepoFullName: undefined,
      });
      vi.spyOn(client, "listReviewComments").mockResolvedValue([]);
      return {
        ...baseRequest(cacheStore),
        client,
        repositoryPath: trancheRepo,
        base: commitSha(trancheBase),
        head: commitSha(trancheHead),
    ...
    /** Counted-mode stdout scoped to one tranche: `files` files reviewed, optional finding. */
    function trancheStdout(
      files: number,
      opts: { tokens?: number; budgetExceeded?: boolean; findingPath?: string } = {},
    ): string {
      const { tokens = 100, budgetExceeded = false, findingPath = undefined } = opts;
      return JSON.stringify({
        status: budgetExceeded ? "failed" : "success",
        summary: { files_reviewed: files, total_tokens: tokens, budget_exceeded: budgetExceeded },
        comments:
          findingPath === undefined
            ...
            : [
                {
                  path: findingPath,
                  content: "This branch never releases the handle it acquired above.",
                  start_line: 1,
                  end_line: 1,
                  severity: "high",
                  category: "bug",
    ...
    function excludesOfCall(index: number): string[] {
      return (runEngineMock.mock.calls[index]?.[0] as { mechanicallyCleanPaths: string[] })
    ...
    it("splits four files into two complementary tranches and completes", async () => {
      const engineDigest = requireEngineDigest();
      runEngineMock
        .mockResolvedValueOnce({ stdout: trancheStdout(2), ruleDigest: engineDigest })
        .mockResolvedValueOnce({ stdout: trancheStdout(2), ruleDigest: engineDigest });

      const report = await performReview(trancheRequest(), createSilentDiagnostics());

      expect(report.outcome).toBe("complete");
      expect(runEngineMock).toHaveBeenCalledTimes(2);
      // Each call excludes exactly the OTHER tranche: the two exclude lists partition the four
      // files, and neither call ever names a file of its own tranche.
      const first = excludesOfCall(0);
      const second = excludesOfCall(1);
      const trancheOf = (excludes: string[]): string[] =>
        ["src/t1.ts", "src/t2.ts", "src/t3.ts", "src/t4.ts"].filter(
          (path) => !excludes.includes(path),
        );
      expect([...trancheOf(first), ...trancheOf(second)].sort()).toEqual([
        "src/t1.ts",
        "src/t2.ts",
        "src/t3.ts",
        "src/t4.ts",
    ...
    it("stops dispatching on the adapter's own real-token check and settles budget_exceeded", async () => {
      const engineDigest = requireEngineDigest();
      // Tranche 1 alone overshoots any allotment this fixture can earn — the adapter must not
      // fund tranche 2, however the engine's own flag behaves.
      runEngineMock.mockResolvedValueOnce({
        stdout: trancheStdout(2, { tokens: 50_000_000 }),
        ruleDigest: engineDigest,
      });

      const diagnostics = createSilentDiagnostics();
      const report = await performReview(trancheRequest(), diagnostics);

      expect(runEngineMock).toHaveBeenCalledTimes(1);
      expect(report.outcome).toBe("incomplete");
      expect(report.reason).toBe("settlement.incomplete.budget_exceeded");
      const codes = diagnostics.drain().map((r) => r.code);
      expect(codes).toContain("engine.tranche_stopped");
    });

    it("keeps a stopped run's finished-tranche verdicts in the cache — exactly those", async () => {
      const engineDigest = requireEngineDigest();
      runEngineMock.mockResolvedValueOnce({
        stdout: trancheStdout(2, { tokens: 50_000_000, findingPath: "src/t1.ts" }),
        ruleDigest: engineDigest,
      });

      const report = await performReview(trancheRequest(EMPTY_STORE), createSilentDiagnostics());

      expect(report.outcome).toBe("incomplete");
      expect(report.reason).toBe("settlement.incomplete.budget_exceeded");
      // Two entries — both tranche-1 paths, none of tranche 2: coverage.completed carries the
      // tranche, and memoizablePaths already reads it — a finished file WITHOUT a finding is
      // cached too (entries are blob-keyed, so the count plus the stored finding's own path are
      // the observable identities).
      expect(report.cacheAppended).toBe(2);
      const storedFindingPaths = (report.updatedCacheStore?.entries ?? [])
        .flatMap((e) => e.findings)
        .map((f) => f.path as string);
      expect(storedFindingPaths).toEqual(["src/t1.ts"]);
    });

    it("discards a tranche the ENGINE budget-stopped — spent, but never believed", async () => {
      const engineDigest = requireEngineDigest();
      runEngineMock
        .mockResolvedValueOnce({ stdout: trancheStdout(2), ruleDigest: engineDigest })
        // Tranche 2 dies on the engine's own budget flag, carrying a finding produced by a
        // half-read file — the exact false-positive class production measured at 18% precision.
        .mockResolvedValueOnce({
          stdout: trancheStdout(1, { budgetExceeded: true, findingPath: "src/t3.ts" }),
          ruleDigest: engineDigest,
        });

      const request = trancheRequest(EMPTY_STORE);
      const created: ReviewCommentInput[] = [];
      ...
      expect(report.outcome).toBe("incomplete");
      expect(report.reason).toBe("settlement.incomplete.budget_exceeded");
      // The half-read file's finding was never published…
      expect(created.some((c) => c.path === "src/t3.ts")).toBe(false);
      // …and its tranche was never covered: only tranche 1's two paths reach the store, and no
      // stored finding names the discarded tranche's file.
      expect(report.cacheAppended).toBe(2);
      const storedFindingPaths = (report.updatedCacheStore?.entries ?? [])
        .flatMap((e) => e.findings)
        .map((f) => f.path as string);
      expect(storedFindingPaths).not.toContain("src/t3.ts");
    });

    it("ends on a failed tranche with today's semantics, keeping earlier findings", async () => {
      const engineDigest = requireEngineDigest();
      ...
      runEngineMock
        .mockResolvedValueOnce({
          stdout: trancheStdout(2, { findingPath: "src/t1.ts" }),
          ruleDigest: engineDigest,
        })
        // Tranche 2 fails twice — once plus its own resume — so the tranche ends failed.
        .mockResolvedValueOnce({ stdout: failedStdout, ruleDigest: engineDigest })
        .mockResolvedValueOnce({ stdout: failedStdout, ruleDigest: engineDigest });
      ...
      // Three calls: tranche 1, tranche 2, tranche 2's own resume — never a third tranche.
      expect(runEngineMock).toHaveBeenCalledTimes(3);
    });
  });
});
